...
/** Close a match left stuck on "live" so a new match can start (never replay a finished game). */
...
function assertRoundRobinMatchCanStart(bracket, matchId, divisionSetup) {
  const schedule = getBracketRoundRobinMatches(
    { ...bracket, scheduleResetAt: divisionSetup?.scheduleResetAt },
    { scheduleResetAt: divisionSetup?.scheduleResetAt }
  );
  const target = schedule.find((m) => m.id === matchId);
  if (!target) {
    throw new Error("Match not found in this bracket schedule.");
  }
  if (!isMatchPlayable(target)) {
    throw new Error(
      "This pairing has already been played. Choose a different match from the schedule."
    );
  }
  const key = roundRobinPairKey(target);
  const duplicatePlayable = schedule.filter(
    (m) => roundRobinPairKey(m) === key && isMatchPlayable(m)
  );
  if (duplicatePlayable.length > 1) {
    throw new Error(
      "Duplicate schedule rows for this pairing. Regenerate this division to fix the bracket."
    );
  }
}
...
export function refreshTournamentStandings(event) {
  ...
  for (const [divisionId, div] of Object.entries(divisions)) {
    const brackets = (div.brackets ?? []).map((b) => {
      ...
      return refreshBracketStandings(
...
export function updateTournamentMatch(
  ...
  if (patch.status === "live") {
    if (bracketMeta) {
      assertRoundRobinMatchCanStart(bracketMeta, matchId, div);
    }
