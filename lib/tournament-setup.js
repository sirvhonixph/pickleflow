...
function alignLiveRowToCanonical(liveRow, canonical) {
  if (!liveRow || !canonical) return liveRow;
  if (
    liveRow.pairAId === canonical.pairAId &&
    liveRow.pairBId === canonical.pairBId
  ) {
    return {
      ...liveRow,
      id: canonical.id,
      pairAId: canonical.pairAId,
      pairBId: canonical.pairBId,
      scheduleOrder: canonical.scheduleOrder ?? liveRow.scheduleOrder,
    };
  }
  if (
    liveRow.pairAId === canonical.pairBId &&
    liveRow.pairBId === canonical.pairAId
  ) {
    return {
      ...liveRow,
      id: canonical.id,
      pairAId: canonical.pairAId,
      pairBId: canonical.pairBId,
      scoreA: liveRow.scoreB ?? 0,
      scoreB: liveRow.scoreA ?? 0,
      scheduleOrder: canonical.scheduleOrder ?? liveRow.scheduleOrder,
    };
  }
  return { ...liveRow, id: canonical.id };
}
...
function buildRegeneratedDivisionSetup(setup, scheduleResetAt, previousBrackets = []) {
  const prevByCourt = new Map(
    (previousBrackets ?? []).filter((b) => b?.courtId).map((b) => [b.courtId, b])
  );
  const brackets = buildRegeneratedBrackets(setup.brackets, scheduleResetAt).map(
    (b) => {
      const prev = prevByCourt.get(b.courtId);
      if (!prev?.id) return b;
      return {
        ...b,
        id: prev.id,
        label: prev.label ?? b.label,
        courtName: prev.courtName ?? b.courtName,
        matches: generateRoundRobinMatches(b.pairIds, prev.id).map((template) => ({
          id: template.id,
          pairAId: template.pairAId,
          pairBId: template.pairBId,
          scheduleOrder: template.scheduleOrder,
  ...
  ).map((b) => purgePreResetBracketRows({ ...b, scheduleResetAt }));
  return refreshDivisionAdvancement({
...
export function applyDivisionSetup(event, divisionId, options = {}) {
  ...
  const divisions = { ...(event.tournamentDivisions ?? {}) };
  const previousBrackets = divisions[divisionId]?.brackets ?? [];
  const setup = buildDivisionSetup(event, divisionId, {
    resetScores: regenerate,
  });
  divisions[divisionId] =
    scheduleResetAt != null
      ? buildRegeneratedDivisionSetup(setup, scheduleResetAt, previousBrackets)
      : setup;
...
export function updateTournamentMatch(
  ...
  const brackets = (div.brackets ?? []).map((bracket) => {
    ...
    if (target && isRoundRobinMatchLocked(target)) {
      if (patch.status === "completed") {
        if (liveRowForCompletion) {
          ...
          const canonicalRow = (stabilizedBase.matches ?? []).find(
            (m) =>
              roundRobinPairKey(m) === roundRobinPairKey(liveRowForCompletion)
          );
          if (canonicalRow) {
            target = alignLiveRowToCanonical(liveRowForCompletion, canonicalRow);
            targetId = canonicalRow.id;
          }
    ...
    const completingLiveSession =
      patch.status === "completed" &&
      liveRowForCompletion &&
      roundRobinPairKey(liveRowForCompletion) === roundRobinPairKey(target);
    ...
    const matches = (stabilizedBase.matches ?? []).map((m) => {
      ...
      if (
        permanent &&
        !completingLiveSession &&
        !(patch.status === "completed" && isMatchLive(m))
      ) {
        return sealPermanentPairingRow(permanent);
      }
      ...
      if (patch.status === "completed") {
        ...
        if (
          !completingLiveSession &&
          !isMatchLive(m) &&
          hasRecordedRoundRobinResult(sealedExisting)
        ) {
