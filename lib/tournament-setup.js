...
export function updateTournamentMatch(
  ...
  const brackets = (div.brackets ?? []).map((bracket) => {
    ...
    const preserveOtherMatchRow = (m) => {
      ...
      const sealed = sealRoundRobinMatchRow(m);
      if (isRoundRobinMatchLocked(sealed) || isRoundRobinMatchLocked(m)) {
        return lockRoundRobinMatchRow(sealed);
      }
      if (
        hasRecordedRoundRobinResult(sealed) &&
        (hasDecisiveRoundRobinScore(sealed) || isForfeitMatch(sealed))
      ) {
        return completeAndLockRoundRobinRow(sealed);
      }
      if (hasRecordedRoundRobinResult(sealed)) {
    ...
    if (startingLive && target) {
      ...
      const alreadyPlayed = (reconciled.matches ?? []).some(
        (m) =>
          ...
          (isRoundRobinMatchLocked(m) || hasRecordedRoundRobinResult(m))
      );
      if (alreadyPlayed) {
        throw new Error(
    ...
    const scheduleResetAt = div.scheduleResetAt ?? bracket.scheduleResetAt;
    const normalizedMatches = sealAllBracketMatchRows(
      getBracketRoundRobinMatches(
        { ...reconciled, matches, scheduleResetAt },
        { scheduleResetAt }
      )
    );
    const withMatches = {
