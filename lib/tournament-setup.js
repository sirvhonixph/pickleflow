...
function sealPermanentPairingRow(m) {
  if (isRoundRobinMatchLocked(m)) {
    return lockRoundRobinMatchRow(m);
  }
  return completeAndLockRoundRobinRow(sealRoundRobinMatchRow(m));
}
...
function finalizeStuckLiveMatch(m, allMatches = [], permanentIndex = null) {
  const permanent =
    resolvePermanentPairingRow(permanentIndex, m) ??
    bestRowForPairing(allMatches, m);
...
function releaseCourtRoundRobinBracket(
  bracket,
  { scheduleResetAt, courtId, divId, divisionId, bracketOrRoundId, matchId }
) {
  ...
  const stabilized = stabilizeBracketMatches(bracket, stabilizeOpts);
  const permanentIndex = buildPermanentPairingIndex(stabilized.matches);

  return stabilizeBracketMatches(
    {
      ...stabilized,
      matches: (stabilized.matches ?? []).map((m) => {
...
export function updateTournamentMatch(
  ...
  const brackets = (div.brackets ?? []).map((bracket) => {
    ...
    const permanentIndex = buildPermanentPairingIndex(stabilizedBase.matches);
