...
/** Close a stuck live match — never wipe a locked/finished row for the same pairing. */
function finalizeStuckLiveMatch(m, allMatches = []) {
  const stored = bestRowForPairing(allMatches, m);
  if (stored && isPermanentRoundRobinResult(stored)) {
    return isRoundRobinMatchLocked(stored)
      ? lockRoundRobinMatchRow(stored)
      : completeAndLockRoundRobinRow(sealRoundRobinMatchRow(stored));
  }
  if (isPermanentRoundRobinResult(m)) {
    return isRoundRobinMatchLocked(m)
      ? lockRoundRobinMatchRow(m)
      : completeAndLockRoundRobinRow(sealRoundRobinMatchRow(m));
  }
    ...
    if (!poolBracketsHiddenByKnockout(nextDiv)) {
      nextDiv = {
        ...
        brackets: (nextDiv.brackets ?? []).map((bracket) => {
          ...
          const rawMatches = bracket.matches ?? [];
          return stabilizeBracketMatches(
            {
              ...
              matches: rawMatches.map((m) => {
                ...
                return finalizeStuckLiveMatch(m, rawMatches);
              }),
            },
            {
        ...
        knockout: {
          ...
          rounds: nextDiv.knockout.rounds.map((round) => ({
            ...
            matches: (round.matches ?? []).map((m) => {
              ...
              return finalizeStuckLiveMatch(m, round.matches ?? []);
            }),
          })),
        },
      };
...
export function applyDivisionSetup(event, divisionId, options = {}) {
  ...
  const brackets =
    scheduleResetAt != null
      ? buildRegeneratedBrackets(setup.brackets, scheduleResetAt)
      : setup.brackets;
  const divisions = { ...(event.tournamentDivisions ?? {}) };
...
export function updateTournamentMatch(
  ...
  if (patch.status === "live") {
    ...
    if (isKnockoutPath) {
      ...
      let row = (bracket?.matches ?? []).find((m) => m.id === matchId);
      if (!row && bracket) {
        row = getBracketRoundRobinMatches(
          { ...bracket, scheduleResetAt: div.scheduleResetAt },
          { scheduleResetAt: div.scheduleResetAt }
        ).find((m) => m.id === matchId);
      }
      startingNewLiveOnCourt = !row || row.status !== "live";
    }
  ...
  const brackets = (div.brackets ?? []).map((bracket) => {
    if (bracket.id !== bracketId) return sanitizeBracket(bracket);
    ...
    const target =
      ...
      (existing
        ...
        : null);
    const targetId = target?.id ?? matchId;
    const startingLive =
    ...
    if (patch.resultLocked === true) {
      ...
      if (isRoundRobinMatchLocked(rowToLock)) {
        return sanitizeBracket(bracket);
      }
      const lockedRow = lockRoundRobinMatchRow(rowToLock);
    ...
    if (startingLive && target) {
      ...
      const alreadyPlayed = (reconciled.matches ?? []).some(
        (m) =>
    ...
    const matches = (reconciled.matches ?? []).map((m) => {
      ...
      if (patch.status === "completed") {
        if (hasRecordedRoundRobinResult(sealRoundRobinMatchRow(m))) {
    ...
    const withMatches = stabilizeBracketMatches(
      { ...bracket, scheduleResetAt, matches },
      { scheduleResetAt, bracketId: bracket.id }
    );
    const refreshed = refreshBracketStandings(withMatches, pairById, {
      scheduleResetAt: div.scheduleResetAt,
    });
