...
function findLiveRowForCompletion(bracket, matchId, existing, target) {
  const rows = bracket.matches ?? [];
  const byId = rows.find((m) => m.id === matchId && m.status === "live");
  if (byId) return byId;
  if (existing?.status === "live" && existing.id === matchId) return existing;
  const key = roundRobinPairKey(target) || roundRobinPairKey(existing);
  if (!key) return null;
  return (
    rows.find(
      (m) => m.status === "live" && m.id === matchId && roundRobinPairKey(m) === key
    ) ??
    (existing?.status === "live" ? existing : null)
  );
}
...
export function applyDivisionSetup(event, divisionId, options = {}) {
  ...
  const brackets =
    scheduleResetAt != null
      ? buildRegeneratedBrackets(setup.brackets, scheduleResetAt).map((b) =>
          purgePreResetBracketRows({ ...b, scheduleResetAt })
        )
      : setup.brackets;
  const divisions = { ...(event.tournamentDivisions ?? {}) };
...
export function updateTournamentMatch(
  ...
  const brackets = (div.brackets ?? []).map((bracket) => {
    ...
    let workingBracket = applyConfirmedResultsToStoredMatches(bracket);
    ...
    if (target && isRoundRobinMatchLocked(target)) {
      if (patch.status === "completed") {
        if (liveRowForCompletion) {
          ...
          const scoreA = patch.scoreA ?? target.scoreA;
          const scoreB = patch.scoreB ?? target.scoreB;
