...
export function updateTournamentMatch(
  ...
  const brackets = (div.brackets ?? []).map((bracket) => {
    ...
    const preserveOtherMatchRow = (m) => {
      ...
      if (hasRecordedRoundRobinResult(sealed)) {
        return {
          ...sealed,
          status: "completed",
          resultLocked: m.resultLocked === true,
          lockedAt: m.lockedAt,
        };
      }
      return m;
    ...
    if (patch.resultLocked === true) {
      ...
      if (isRoundRobinMatchLocked(target)) {
        return sanitizeBracket(bracket);
      }
      const lockedRow = lockRoundRobinMatchRow(target);
      const targetKey = roundRobinPairKey(target);
