...
export async function fetchEventById(id) {
  ...
  try {
    ...
    throw new Error(
      "Cannot reach the app server. In the Pickleflow folder run: npm run dev — then open the URL it shows (e.g. http://localhost:3000/events/" +
        id +
        ")."
    );
  }
...
export async function registerForEvent(eventId, player, tournamentEntry = null) {
  ...
  const body = {
    ...
    email: player.email,
    registrantName: player.name ?? player.email,
    ...(tournamentEntry ?? {}),
    category:
...
export async function addCourt(eventId, label) {
  ...
  const data = await parseJson(
    await fetch(`/api/events/${eventId}/court`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ label, hostId }),
    })
  );
  let ev = normalizeEvent(data.event);
  if (ev?.type === "open_play") {
    ev = await processEventAutomation(eventId);
  }
  return ev;
}
