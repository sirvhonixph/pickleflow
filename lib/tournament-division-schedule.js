...
export function assertCanBracketDivision(
  ...
  if (existing && isDivisionComplete(existing)) {
    throw new Error(
      `${divisionLabel(divisionId, event)} is finished. Its results are kept for this tournament.`
    );
  }
  ...
  if (
    ...
    throw new Error(
      `${divisionLabel(divisionId, event)} already has match results. Use regenerate with force to erase scores and rebuild brackets.`
    );
  }
