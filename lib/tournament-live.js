...
export function isMatchComplete(match) {
  ...
  if (match.playedAt != null) return true;
  const scoreA = match.scoreA ?? 0;
  const scoreB = match.scoreB ?? 0;
  return scoreA > 0 || scoreB > 0;
}

/** True if this pairing has not been played yet (round robin = once per pair-up). */
export function isMatchPlayable(match) {
  if (!match?.pairAId || !match?.pairBId) return false;
  if (isMatchLive(match)) return false;
  if (match.status === "completed" || match.playedAt != null) return false;
  const scoreA = match.scoreA ?? 0;
  const scoreB = match.scoreB ?? 0;
  if (scoreA > 0 || scoreB > 0) return false;
  return !matchCountsForStandings(match);
}
