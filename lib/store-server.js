  ...
  try {
    do {
      const page = await list({
        prefix: EVENT_BLOB_PREFIX,
        limit: 1000,
        cursor,
      });
      for (const blob of page.blobs ?? []) {
        const id = eventIdFromBlobPathname(blob.pathname);
        if (id) ids.add(id);
      }
      cursor = page.hasMore ? page.cursor : undefined;
    } while (cursor);
  } catch (err) {
    console.error("recoverEventIdsFromBlobStorage failed:", err);
  }
  return [...ids];
}

async function emptyBlobIndex() {
  return {
...
async function recoverBlobIndex(parsed = null) {
  const fromBlobs = await recoverEventIdsFromBlobStorage();
  let repoEvents = [];
  try {
    const parsedFile = await readStoreFileRaw();
    repoEvents = (parsedFile.events ?? []).filter(isFullEventRecord);
  } catch (err) {
    console.error("recoverBlobIndex repo read failed:", err);
  }

  const mergedIds = [
    ...new Set([
      ...fromBlobs,
      ...repoEvents.map((ev) => ev.id).filter(Boolean),
    ]),
  ];

  if (mergedIds.length > 0) {
    for (const ev of repoEvents) {
      if (!mergedIds.includes(ev.id)) continue;
      const exists = await blobEventExists(ev.id);
      if (exists !== "ok") {
        await safeWriteEventBlob(ev);
      }
    }
    const store = {
      storeVersion: STORE_VERSION,
    ...
    try {
      ...
      console.error("recoverBlobIndex write failed:", err);
    }
    return store;
  }

  try {
    return await seedStoreFromFileIfPresent();
  } catch (err) {
    console.error("recoverBlobIndex seedStoreFromFileIfPresent failed:", err);
    return emptyBlobIndex();
  }
}
  ...
  try {
    ...
    if (result?.statusCode === 200 && result.stream) {
      ...
      try {
        ...
        console.error("readStoreBlobRaw index JSON parse failed:", parseErr);
        return recoverBlobIndex();
      }
      if (!Array.isArray(parsed.eventIds) || parsed.eventIds.length === 0) {
        const recovered = await recoverEventIdsFromBlobStorage();
        if (recovered.length > 0) {
          ...
          try {
            ...
            console.error("readStoreBlobRaw index rebuild failed:", writeErr);
          }
        } else {
          return recoverBlobIndex(parsed);
        }
      }
...
async function migrateStoreToSplit(parsed) {
  ...
  if (parsed.storeVersion === STORE_VERSION && fullEvents.length === 0) {
    return parsed;
  }
...
async function readBlobIndexRaw() {
  ...
  if (!(parsed.eventIds ?? []).length) {
    parsed = await recoverBlobIndex(parsed);
  }
...
async function readFullStore() {
  ...
  async function loadFromIndex(raw) {
    const eventIds = [...new Set((raw.eventIds ?? []).filter(Boolean))];
    await seedRepoEventFiles(eventIds);
    const events = [];
    for (const id of eventIds) {
      try {
        ...
        console.error("readEventRecord failed:", id, err);
      }
    }
  ...
  let raw;
  try {
    raw = await readBlobIndexRaw();
  } catch (err) {
    console.error("readBlobIndexRaw failed:", err);
    raw = await recoverBlobIndex();
  }

  let store = await loadFromIndex(raw);
  if ((store.events ?? []).length === 0) {
    raw = await recoverBlobIndex(raw);
    store = await loadFromIndex(raw);
  }
  return store;
}
...
export async function getEventById(id, { refresh = true, persistRepair = true } = {}) {
  try {
    ...
    if (
      ...
      try {
        const latest = await readEventRecord(id);
        const repaired = normalizeEvent(
          mergeConcurrentEventWrites(
            normalizeEvent(latest ?? normalized),
            refreshed
          )
        );
        await writeEventRecord(repaired);
      } catch (err) {
...
export async function updateEventRecord(
  ...
  return withEventWriteLock(id, async () => {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      const beforeCourtIds = courtIds(current);

      const raw =
        typeof updater === "function"
          ? updater(current)
          : { ...current, ...updater };
      let regenNext = normalizeEvent(raw);
      if (current.status === "ended") {
        regenNext = {
          ...regenNext,
          status: "ended",
          endedAt: regenNext.endedAt ?? current.endedAt ?? null,
          tournamentPhase:
            regenNext.tournamentPhase === "ended"
              ? "ended"
      ...
      const addedPairIds = newPairRegistrationIds(current, next);
      const requiredPairIds = new Set([...beforePairIds, ...addedPairIds]);
      const addedCourtIds = newCourtIds(current, next);
      const requiredCourtIds = new Set([...beforeCourtIds, ...addedCourtIds]);

      const mergeWithRegenGuard = (freshEvent, mergedNext) =>
        applyRegeneratedDivisionSnapshots(
          current,
          normalizeEvent(
            mergeConcurrentEventWrites(normalizeEvent(freshEvent), mergedNext)
          ),
          regenNext
        );
      ...
      if (fresh) {
        next = mergeWithRegenGuard(fresh, next);
        if (refreshTournament) next = refreshTournamentIfNeeded(next);
      }

      await writeEventRecord(next);

      let verify = await readEventRecord(id);
      if (!verify) return normalizeEvent(next);

      let merged = mergeWithRegenGuard(verify, next);

      for (let repair = 0; repair < 4; repair++) {
        if (
          eventHasAllPairIds(verify, requiredPairIds) &&
          eventHasAllCourtIds(verify, requiredCourtIds)
        ) {
          break;
        }
        merged = mergeWithRegenGuard(verify, next);
        await writeEventRecord(merged);
        if (repair < 3) {
          await new Promise((resolve) => setTimeout(resolve, 40 * (repair + 1)));
        }
        verify = await readEventRecord(id);
        if (!verify) break;
      }

      merged = mergeWithRegenGuard(verify ?? merged, next);

      if (
        eventHasAllPairIds(verify ?? merged, requiredPairIds) &&
        eventHasAllCourtIds(verify ?? merged, requiredCourtIds)
      ) {
        if (
          !eventHasAllPairIds(verify ?? merged, pairRegistrationIds(merged)) ||
          !eventHasAllCourtIds(verify ?? merged, courtIds(merged))
        ) {
          await writeEventRecord(merged);
        }
