  ...
  return withEventWriteLock(id, async () => {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      for (let repair = 0; repair < 4; repair++) {
        if (eventHasAllPairIds(verify, requiredPairIds)) break;
      ...
      if (eventHasAllPairIds(verify ?? merged, requiredPairIds)) {
        if (!eventHasAllPairIds(verify ?? merged, pairRegistrationIds(merged))) {
          await writeEventRecord(merged);
        }
