...
async function readEventRecord(eventId) {
  const fromSplit = await readEventBlobRaw(eventId);
  if (fromSplit) return fromSplit;

  const raw = await readStoreRaw();
  if (raw.storeVersion === STORE_VERSION) return null;

  const legacy = (raw.events ?? []).find((e) => e.id === eventId);
  if (legacy && isFullEventRecord(legacy)) {
    await writeEventBlobRaw(legacy);
    return legacy;
  }
  return legacy ?? null;
}
...
export async function readStore() {
  ...
  if (raw.storeVersion === STORE_VERSION) {
    const events = (
      await Promise.all((raw.eventIds ?? []).map((id) => readEventBlobRaw(id)))
    ).filter(Boolean);
...
export async function getEventById(id, { refresh = true } = {}) {
  try {
    const event = await readEventRecord(id);
    if (!event) return null;
    const normalized = normalizeEvent(event);
    return refresh ? refreshTournamentIfNeeded(normalized) : normalized;
  } catch {
    return null;
  }
}
