...
async function recoverBlobIndex(parsed = null) {
  const recoveredIds = await recoverEventIdsFromBlobStorage();
  ...
  if (process.env.VERCEL) {
    try {
      ...
      const migrated = await migrateStoreToSplit(parsedFile);
      await writeBlobIndexRaw(migrated);
      await seedRepoEventFiles(migrated.eventIds);
      return migrated;
    } catch (err) {
      console.error("recoverBlobIndex repo migration failed:", err);
    }
  }
  try {
