...
function divisionScheduleResets(event) {
  const out = {};
  for (const [id, div] of Object.entries(event?.tournamentDivisions ?? {})) {
    out[id] = div?.scheduleResetAt ?? 0;
  }
  return out;
}

/** After regenerate, never let concurrent blob merges reattach pre-reset bracket rows. */
export function applyRegeneratedDivisionSnapshots(before, merged, authoritative) {
  if (!authoritative?.tournamentDivisions || !merged) return merged;
  const beforeResets = divisionScheduleResets(before);
  const divisions = { ...(merged.tournamentDivisions ?? {}) };
  let changed = false;
  for (const [id, authDiv] of Object.entries(authoritative.tournamentDivisions)) {
    const authReset = authDiv?.scheduleResetAt ?? 0;
    if (authReset > (beforeResets[id] ?? 0)) {
      divisions[id] = authDiv;
      changed = true;
    }
  }
  return changed ? { ...merged, tournamentDivisions: divisions } : merged;
}
...
export function applyEventFetch(local, remote) {
  ...
  if (!fetchLooksStale(local, remote)) {
    merged = {
      ...remote,
      tournamentDivisions: mergeTournamentDivisions(local, remote),
    };
  } else {
    merged = mergeEventSnapshots(local, remote);
  }
  return applyRegeneratedDivisionSnapshots(local, merged, remote);
}
