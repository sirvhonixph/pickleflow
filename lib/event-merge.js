...
/**
 * Apply a polled/fetched event without dropping pairs from a stale server read.
...
export function applyEventFetch(local, remote) {
  ...
  const localIds = pairRegistrationIds(local);
  const remoteIds = pairRegistrationIds(remote);
