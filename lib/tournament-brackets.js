...
export function recordBracketConfirmedResult(bracket, row) {
  if (row?.status === "live") return bracket;
  if (!isPermanentRoundRobinResult(row)) return bracket;
  const key = roundRobinPairKey(row);
  if (!key) return bracket;
  const scheduleResetAt = bracket?.scheduleResetAt ?? null;
  const sealed = sealPermanentPairingRow({
    ...row,
    scheduleGeneration:
      row.scheduleGeneration ?? scheduleResetAt ?? row.scheduleGeneration,
  });
...
/** True when a row belongs to a schedule generation before the latest division reset. */
export function isStaleScheduleGeneration(row, scheduleResetAt) {
  if (scheduleResetAt == null) return false;
  if (row?.scheduleGeneration == null) return false;
  return row.scheduleGeneration !== scheduleResetAt;
}

/** True when a row belongs to a schedule generation before the latest division reset. */
export function isPreResetMatchRow(m, scheduleResetAt) {
  if (scheduleResetAt == null) return false;
  if (m?.scheduleGeneration === scheduleResetAt) return false;
  if (isMatchLive(m)) return isStaleScheduleGeneration(m, scheduleResetAt) || m?.scheduleGeneration == null;
  if (m?.scheduleGeneration == null) return false;
  return m.scheduleGeneration !== scheduleResetAt;
}

function stampCurrentScheduleGeneration(row, scheduleResetAt) {
  if (scheduleResetAt == null || row == null) return row;
  if (isPermanentRoundRobinResult(row) || isRoundRobinMatchLocked(row)) {
    return { ...row, scheduleGeneration: scheduleResetAt };
  }
  if (row.scheduleGeneration == null) {
    return { ...row, scheduleGeneration: scheduleResetAt };
  }
  return row;
}

export function purgePreResetBracketRows(bracket) {
  const scheduleResetAt = bracket?.scheduleResetAt ?? null;
  if (scheduleResetAt == null) return bracket;
  const confirmedResults = Object.fromEntries(
    Object.entries(bracket?.confirmedResults ?? {})
      .filter(([, row]) => !isStaleScheduleGeneration(row, scheduleResetAt))
      .map(([key, row]) => [
        key,
        stampCurrentScheduleGeneration(row, scheduleResetAt),
      ])
  );
  const matches = (bracket?.matches ?? [])
    .filter((m) => !isPreResetMatchRow(m, scheduleResetAt))
    .map((m) => stampCurrentScheduleGeneration(m, scheduleResetAt));
  return { ...bracket, confirmedResults, matches };
}

export function syncBracketConfirmedResults(bracket) {
  const scheduleResetAt = bracket?.scheduleResetAt ?? null;
  const purged = purgePreResetBracketRows(bracket);
  let next = {
    ...purged,
    confirmedResults: { ...(purged.confirmedResults ?? {}) },
  };
  for (const row of purged.matches ?? []) {
    if (isMatchLive(row)) continue;
    if (
      scheduleResetAt != null &&
      row?.scheduleGeneration != null &&
...
export function applyConfirmedResultsToStoredMatches(bracket) {
  ...
  const matches = (synced.matches ?? []).map((m) => {
    ...
    if (
      confirmed &&
      isPermanentRoundRobinResult(confirmed) &&
      (synced.scheduleResetAt == null ||
        confirmed.scheduleGeneration === synced.scheduleResetAt)
    ) {
      const stamped = stampCurrentScheduleGeneration(
        confirmed,
        synced.scheduleResetAt
      );
      return {
        ...sealPermanentPairingRow(stamped),
...
function orphanLiveRowToStored(m, permanentIndex) {
...
/** Only one live RR match per court — fixes stale blob/client merge duplicates. */
export function sanitizeBracketCourtLiveMatches(bracket) {
  if (!bracket?.courtId) return bracket;
  const matches = bracket.matches ?? [];
  const liveRows = matches.filter((m) => isMatchLive(m));
  if (liveRows.length <= 1) return bracket;

  const keep = [...liveRows].sort(
    (a, b) => (b.startedAt ?? 0) - (a.startedAt ?? 0)
  )[0];
  const keepKey = roundRobinPairKey(keep);
  ...
  return {
    ...
    matches: matches.map((m) => {
      if (!isMatchLive(m)) return m;
      if (m.id === keep.id || roundRobinPairKey(m) === keepKey) return m;
      return orphanLiveRowToStored(m, permanentIndex);
    }),
  };
}

/** Merge-aware row pick — stale live 0-0 must not beat a released scheduled row. */
export function mergePreferredRoundRobinRow(prev, next) {
  if (
    isMatchLive(prev) &&
    !isMatchLive(next) &&
    ...
    (prev.scoreA ?? 0) === 0 &&
    (prev.scoreB ?? 0) === 0 &&
    !hasDecisiveRoundRobinScore(prev)
  ) {
    return next;
  }
  if (
    ...
    (next.scoreA ?? 0) === 0 &&
    (next.scoreB ?? 0) === 0 &&
    !hasDecisiveRoundRobinScore(next)
  ) {
    return prev;
  }
  return pickPreferredRoundRobinRow(prev, next);
}
...
export function stabilizeBracketMatches(bracket, options = {}) {
  const scheduleResetAt =
    options.scheduleResetAt ?? bracket?.scheduleResetAt ?? null;
  const synced = sanitizeBracketCourtLiveMatches(
  ...
  const ingest = (row, fromConfirmed = false) => {
    ...
    if (isStaleMatchResult(row, scheduleResetAt)) return;
    const key = roundRobinPairKey(row);
...
export function buildRegeneratedBrackets(brackets, scheduleResetAt) {
  ...
  return (brackets ?? []).map((bracket) => ({
    ...
    scheduleResetAt,
    confirmedResults: {},
    matches: generateRoundRobinMatches(bracket.pairIds, bracket.id).map((m) => ({
      id: m.id,
      pairAId: m.pairAId,
      pairBId: m.pairBId,
    ...
    advancedPairIds: [],
    poolComplete: false,
    roundRobinMeta: bracket.roundRobinMeta
      ? {
          ...bracket.roundRobinMeta,
          finishedMatches: 0,
          matchesRemaining: bracket.roundRobinMeta.matchCount,
        }
      : bracket.roundRobinMeta,
  }));
}
...
function isStaleMatchResult(m, scheduleResetAt) {
  return isPreResetMatchRow(m, scheduleResetAt);
}
...
export function mergeRoundRobinSchedule(
  ...
  for (const row of Object.values(confirmedResults ?? {})) {
    ...
    if (isStaleMatchResult(row, scheduleResetAt)) continue;
    const key = roundRobinPairKey(row);
  ...
  for (const m of dedupeRoundRobinMatches(existingMatches ?? [])) {
    ...
    if (isStaleMatchResult(m, scheduleResetAt)) continue;
    const key = roundRobinPairKey(m);
  ...
  const merged = canonical.map((template) => {
    ...
    if (
      confirmed &&
      isPermanentRoundRobinResult(confirmed) &&
      !isStaleMatchResult(confirmed, scheduleResetAt)
    ) {
