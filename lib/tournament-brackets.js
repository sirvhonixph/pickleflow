...
export function purgePreResetBracketRows(bracket) {
  ...
  const confirmedResults = Object.fromEntries(
    Object.entries(bracket?.confirmedResults ?? {}).filter(
      ([, row]) => row?.scheduleGeneration === scheduleResetAt
    )
  );
  const matches = (bracket?.matches ?? []).filter(
    (m) => !isPreResetMatchRow(m, scheduleResetAt)
  );
...
export function syncBracketConfirmedResults(bracket) {
  ...
  for (const row of purged.matches ?? []) {
    if (scheduleResetAt != null && row?.scheduleGeneration !== scheduleResetAt) {
      continue;
    }
