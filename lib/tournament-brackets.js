...
export function isPreResetMatchRow(m, scheduleResetAt) {
  ...
  // Final results are never dropped — stale generation tags are repaired instead.
  if (isPermanentRoundRobinResult(m) || isRoundRobinMatchLocked(m)) {
    return false;
  }
  ...
  const matches = (synced.matches ?? []).map((m) => {
    ...
    if (
      ...
      (synced.scheduleResetAt == null ||
        confirmed.scheduleGeneration == null ||
