...
export function getBracketRoundRobinMatches(bracket, options = {}) {
  return mergeRoundRobinSchedule(bracket?.matches, bracket?.pairIds, {
    scheduleResetAt: bracket?.scheduleResetAt ?? options.scheduleResetAt,
  });
}

function roundRobinRowRank(m) {
  if (isMatchComplete(m)) return 4;
  if (isMatchLive(m)) return 3;
...
export function pickPreferredRoundRobinRow(prev, next) {
  const rp = roundRobinRowRank(prev);
...
/** Drop pre-reset results only; never discard live or post-reset games. */
function isStaleMatchResult(m, scheduleResetAt) {
  ...
  const anchor = Math.max(m.playedAt ?? 0, m.startedAt ?? 0);
  if (m.status === "live" || anchor >= scheduleResetAt) return false;
  if (m.status === "completed" || m.winnerPairId || m.playedAt != null) return true;
  const scoreA = m.scoreA ?? 0;
  const scoreB = m.scoreB ?? 0;
...
export function mergeRoundRobinSchedule(existingMatches, pairIds, options = {}) {
  const ids = [...new Set((pairIds ?? []).filter(Boolean))];
  const canonical = generateRoundRobinMatches(ids);
  const { resetScores = false, scheduleResetAt } = options;
  ...
  for (const m of dedupeRoundRobinMatches(existingMatches ?? [])) {
    ...
    const key = roundRobinPairKey(m);
    let saved = normalizeStoredMatch(m);
    if (isStaleMatchResult(saved, scheduleResetAt)) continue;
    if (isVoidMatchResult(saved)) {
      saved = reopenMatchForRematch(saved);
    }
    const prev = byKey.get(key);
  ...
  const merged = canonical.map((template) => {
    ...
    let row = saved ? normalizeStoredMatch(saved) : template;
    if (isVoidMatchResult(row)) {
      row = reopenMatchForRematch({ ...row, ...template, id: template.id });
    }
    if (!isCanonicalRoundRobinId(row.id) && isCanonicalRoundRobinId(template.id)) {
      row = { ...row, id: template.id };
    }
    return row;
  });
  const out = dedupeRoundRobinMatches(merged);
  out.sort((a, b) => roundRobinPairKey(a).localeCompare(roundRobinPairKey(b)));
  return out;
}
