...
function isStaleMatchResult(m, scheduleResetAt) {
  ...
  const playedAt = m.playedAt ?? m.startedAt ?? 0;
  if (m.status === "completed" || m.winnerPairId) {
    return playedAt < scheduleResetAt;
  }
  const scoreA = m.scoreA ?? 0;
  const scoreB = m.scoreB ?? 0;
  if (scoreA > 0 || scoreB > 0) return playedAt < scheduleResetAt;
  return false;
}
