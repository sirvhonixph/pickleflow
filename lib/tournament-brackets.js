...
export function buildPermanentPairingIndex(matches) {
  const index = new Map();
  for (const row of matches ?? []) {
    if (!isPermanentRoundRobinResult(row)) continue;
    const key = roundRobinPairKey(row);
    if (!key) continue;
    const prev = index.get(key);
    index.set(key, prev ? pickPreferredRoundRobinRow(prev, row) : row);
  }
  return index;
}
...
export function stabilizeBracketMatches(bracket, options = {}) {
  ...
  const merged = getBracketRoundRobinMatches(
    ...
    { scheduleResetAt, bracketId: bracket?.id ?? null }
  );
  const byKey = new Map();
  const ingest = (row) => {
    ...
    byKey.set(key, prev ? pickPreferredRoundRobinRow(prev, row) : row);
  };
  for (const row of bracket?.matches ?? []) ingest(row);
  for (const row of merged) ingest(row);

  const stable = [...byKey.values()].map((m) => {
