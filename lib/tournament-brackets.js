...
export function isPreResetMatchRow(m, scheduleResetAt) {
  ...
  if (isPermanentRoundRobinResult(m) || isRoundRobinMatchLocked(m)) {
    return (
      m?.scheduleGeneration != null && m.scheduleGeneration !== scheduleResetAt
    );
  }
  return true;
}
...
function stampCurrentScheduleGeneration(row, scheduleResetAt) {
  ...
  if (
    (isPermanentRoundRobinResult(row) || isRoundRobinMatchLocked(row)) &&
...
export function purgePreResetBracketRows(bracket) {
  ...
  const confirmedResults = Object.fromEntries(
    Object.entries(bracket?.confirmedResults ?? {})
      .filter(([, row]) => {
        if (row?.scheduleGeneration === scheduleResetAt) return true;
        if (
