...
export function canRegisterAnotherTournamentEntry(event, playerId, playerName) {
  ...
  const total = getTournamentPairCountForPlayer(event, playerId);
  if (total >= MAX_ENTRIES_PER_NAME_PER_CATEGORY) return false;
  ...
  return total < MAX_ENTRIES_PER_NAME_PER_CATEGORY;
}
