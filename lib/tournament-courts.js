...
export function buildCourtRecord(event, label) {
  const num = (event.courts?.length ?? 0) + 1;
  return {
    id: `court-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
export function addCourtToEvent(event, label) {
  return {
    ...event,
    courts: [...(event.courts ?? []), buildCourtRecord(event, label)],
  };
}

export function assertCanRemoveOpenPlayCourt(event, courtId) {
