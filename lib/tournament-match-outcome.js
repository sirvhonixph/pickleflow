...
function normalizeLockedRoundRobinRow(match) {
...
export function lockRoundRobinMatchRow(match) {
  if (isRoundRobinMatchLocked(match)) {
    return normalizeLockedRoundRobinRow(match);
  }
  const sealed = sealRoundRobinMatchRow(match);
  if (!hasRecordedRoundRobinResult(sealed)) {
    throw new Error(
      "This match does not have a final score yet. End the match on court first."
    );
  }
  return normalizeLockedRoundRobinRow({ ...sealed, lockedAt: match.lockedAt });
}

/** Normalize a permanent row for storage / merge. */
export function sealPermanentPairingRow(match) {
  if (isRoundRobinMatchLocked(match)) {
    return lockRoundRobinMatchRow(match);
  }
  return completeAndLockRoundRobinRow(sealRoundRobinMatchRow(match));
}

/** End-of-match: seal a decisive result and lock it in one step. */
export function completeAndLockRoundRobinRow(match) {
  const sealed = sealRoundRobinMatchRow(match);
  if (!hasRecordedRoundRobinResult(sealed)) return sealed;
  return normalizeLockedRoundRobinRow({ ...sealed, lockedAt: Date.now() });
}
...
export function isVoidMatchResult(match) {
  if (!match?.pairAId || !match?.pairBId) return false;
  if (isPermanentRoundRobinResult(match)) return false;
...
export function sealRoundRobinMatchRow(match) {
  ...
  if (isRoundRobinMatchLocked(match)) return normalizeLockedRoundRobinRow(match);
...
export function sealAllBracketMatchRows(matches) {
  return (matches ?? []).map((m) => {
    if (isRoundRobinMatchLocked(m)) {
      return normalizeLockedRoundRobinRow(m);
    }
    if (m?.status === "live") {
      return m;
    }
...
/** True when a row must never be reopened for rematch. */
export function isPermanentRoundRobinResult(match) {
  if (!match?.pairAId || !match?.pairBId) return false;
  if (isRoundRobinMatchLocked(match)) return true;
  if (isForfeitMatch(match)) return true;
  if (hasDecisiveRoundRobinScore(match)) return true;
  const sealed = sealRoundRobinMatchRow(match);
  return hasRecordedRoundRobinResult(sealed);
}

/** Only void / invalid rows may reopen — recorded / locked results are permanent. */
export function reopenMatchForRematch(match) {
  if (isPermanentRoundRobinResult(match)) {
    if (isRoundRobinMatchLocked(match)) {
      return normalizeLockedRoundRobinRow(match);
    }
    const sealed = sealRoundRobinMatchRow(match);
